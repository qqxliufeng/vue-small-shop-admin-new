<template>
  <el-card
    ...
    class="s-d-t-type-container"
    v-if="typeGoodsList && typeGoodsList.length > 0"
  >
    <div
      class="s-d-t-type-title-wrapper"
      ref="type"
      @click="positionType"
    >
    ...
    <div>
      ...
      >
        <swiper-slide
          v-for="(tabItem, index) of tempTypeGoodsList"
          :key="tabItem.goodsTypeId"
        >
          <span
            class="tab-item"
            @click="selectTabItem(index)"
            :class="{'tab-item-selected' : tabItem.isSelected}"
          >{{tabItem.goodsTypeName}}</span>
        </swiper-slide>
      </swiper>
      <ul
        v-if="currentTabItems && currentTabItems.length > 0"
        class="ticket-wrapper"
      >
        <li
          v-for="item of tempCurrentTabItems"
          :key="item.goodsId"
        >
          <scenic-detail-ticket-item
            :item="item"
            @reseve-detail="reseveDetail"
            @share-ticket="shareTicket"
            @show-tip-dialog="shareTipDialog"
          ></scenic-detail-ticket-item>
        </li>
        <div
          v-if="tempCurrentTabItems && tempCurrentTabItems.length < currentTabItems.length"
          class="show-more"
          @click="showMore"
        >
          加载更多
...
.dialog-tip-wrapper {
  ...
  .dialog-content {
    ...
    .content-top {
      ...
      border-top-left-radius: rem(0.1);
      border-top-right-radius: rem(0.1);
      position: relative;
      display: flex;
      flex-direction: column;
      overflow: hidden;
